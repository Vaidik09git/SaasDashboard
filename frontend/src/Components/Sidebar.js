...
const Sidebar = ({ isOpen, toggleSidebar, isFileUploaded }) => {
  const menuItems = [
    { icon: <Home size={20} />, label: 'Home', active: true, protected: false },
    { icon: <BarChart2 size={20} />, label: 'Performance', active: false, protected: true },
    { icon: <ClipboardList size={20} />, label: 'Audit Logs', active: false, protected: true },
    { icon: <Download size={20} />, label: 'Export Center', active: false, protected: true },
    { icon: <Users size={20} />, label: 'Team Workspace', active: false, protected: false },
    { icon: <Settings size={20} />, label: 'Settings', active: false, protected: false },
  ];

  const handleNavClick = (item) => {
    // Blocks access to specific bars if no file is uploaded
    if (item.protected && !isFileUploaded) {
      alert("⚠️ Please upload a file or choose a project first to access " + item.label);
      return;
    }
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen bg-[#0f172a] border-r border-white/10 z-50 transition-all duration-300 ease-in-out flex flex-col shadow-2xl ${isOpen ? 'w-72 p-6' : 'w-20 p-4'}`}>
      ...
      <nav className="space-y-1 mb-6 overflow-y-auto overflow-x-hidden custom-scrollbar flex-none pr-1">
        ...
        {menuItems.map((item) => (
          <button 
            ...
            className={`flex items-center gap-4 py-2.5 rounded-xl transition-all group ${
                item.active ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'text-slate-300 hover:bg-slate-800 hover:text-white'
